<div>
                    ...
                    <div class="d-flex justify-content-between align-items-center flex-column flex-lg-row">
                        <ul class="nav nav-pills align-items-center">
                            <li class="nav-item mr-3">
                                <button class="nav-link bg-danger text-white" wire:click="refresh_all" wire:loading.attr="disabled">
                                    {{ __('Refresh All') }}
                                </button>
                            </li>
                            <li class="nav-item">
                                <button class="nav-link @if ($status == 'all') active @endif"
                                    ...
                                    {{ __('All') }}
                                    <span class="badge badge-primary">{{ $social_total }}</span>
                                </button>
                            </li>
                            <li class="nav-item">
                                <button class="nav-link @if ($status == 'drafted') active @endif"
                                    ...
                                    {{ __('Drafted') }}
                                    <span class="badge badge-primary">{{ $social_drafted }}</span>
                                </button>
                            </li>
                            <li class="nav-item mr-3">
                                <button class="nav-link @if ($status == 'published') active @endif"
                                    ...
                                    {{ __('Published') }}
                                    <span class="badge badge-primary">{{ $social_published }}</span>
                                </button>
                            </li>
                        </ul>
                        <a class="mt-3 mt-lg-0 btn btn-primary" href="{{ route('dashboard.social.create') }}">
    ...
    <div class="row mt-3">
        @forelse($socials as $social)
            <livewire:backend.social.item :social="$social" :wire:key="rand()" />
        @empty
            <div class="col-12">
